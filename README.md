# ☁️ NEONRUNNER

**NEONRUNNER** is a relaxing, pastel-themed endless platformer game built with HTML5 Canvas and Vanilla JavaScript. Jump across platforms, collect coins, and survive as the speed increases!
<img width="1822" height="1184" alt="image" src="https://github.com/user-attachments/assets/bc7bb4f0-8a96-4cfe-bce8-b136733fcb2f" />
<img width="1822" height="1184" alt="image" src="https://github.com/user-attachments/assets/b98879ba-e24c-47ef-9161-9e7c0a5331a9" />




## 🎮 Play Now

You can play the game directly in your browser here:
### [👉 Click to Play NEORUNNER](https://cyborgwastaken.github.io/GICSubmission/)

## ✨ Features

* **Endless Gameplay:** The game generates platforms indefinitely. How high can you score?
* **Dynamic Difficulty:** As you level up, the game speeds up, platforms get further apart, and your jump strength increases to compensate.
* **Cyberpunk Aesthetic:** Features a "Dark Blue" background animation and clean UI.
* **Coin System:** Collect gold coins for bonus points.
* **Responsive Design:** The game canvas automatically resizes to fit your window.

## 🕹️ Controls

The game supports both Arrow keys and WASD.

| Action | Key 1 | Key 2 | Key 3 |
| :--- | :---: | :---: | :---: |
| **Move Left** | `← Arrow Left` | `A` | |
| **Move Right** | `→ Arrow Right` | `D` | |
| **Jump** | `Spacebar` | `↑ Arrow Up` | `W` |

## 🛠️ Installation & Setup

Since this is a static web project, you don't need to install any dependencies.

1.  **Clone the repository:**
    ```bash
    git clone https://github.com/cyborgwastaken/GICSubmission.git
    ```
2.  **Run locally:**
    Simply double-click the `game.html` file to open it in your default web browser.

## 💻 Tech Stack

* **HTML5 Canvas:** For rendering high-performance 2D graphics.
* **CSS3:** For UI styling and the background keyframe animations.
* **Vanilla JavaScript:** Game logic, physics, and collision detection (No external libraries used).

## 🚀 How to Deploy to GitHub Pages

1.  Go to your repository on GitHub.
2.  Click on **Settings**.
3.  On the left sidebar, click **Pages**.
4.  Under **Build and deployment**, select **main** (or master) branch as the source.
5.  Click **Save**.
6.  Wait a minute, and your link will appear at the top of the page!

## 📝 License

This project is open source and available under the [MIT License](LICENSE).

---
*Created by Ayushman Das*
